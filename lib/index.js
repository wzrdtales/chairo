...
module.exports = {
    register: async (server, options) => {
        const mim = await import('microwizard');
        const MicroWizard = mim.default
        const {convertData} = mim;

        const settings = Hoek.applyToDefaults(internals.defaults, options);

        const hasMicroWizard =
            settings.seneca && typeof settings.seneca.use === 'function';
        const seneca = hasMicroWizard ? settings.seneca : new MicroWizard(settings);
        ...
        server.decorate('server', 'jsonic', convertData);
...
internals.action = function (server) {
    ...
    return function (name, pattern, options) {
        ...
        if (typeof pattern === 'string') {
            pattern = server.jsonic(pattern).o;
        }
...
internals.cache = {
    generateKey: function (additions) {
        ...
        if (typeof additions === 'string') {
            additions = this.server.jsonic(additions).o;
        }
