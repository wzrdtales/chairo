...
internals.toolkit.act = async function (pattern) {

    try {
        const result = await this.request.seneca.actAsync(pattern);
...
internals.toolkit.compose = async function (template, context, options) {
    ...
    try {
        await Promise.all(actions.map((action) => each(action)));
