...
module.exports = {
  register: (server, options) => {
    ...
    server.decorate('toolkit', 'act', internals.replies.act);
    server.decorate('toolkit', 'compose', internals.replies.compose);
      ...
      if (additions) {
        return server.seneca.act(
          Hoek.applyToDefaults(
            pattern,
            typeof additions === 'string' ? Jsonic(additions) : additions
          ),
          callback
        );
      }
